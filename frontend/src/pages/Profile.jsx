    ...
    if (window.confirm("Sei sicuro di voler eliminare il tuo account? Questa azione è irreversibile.")) {
      try {
        ...
        const res = await axios.delete(
          `https://webeliteblogenzo.onrender.com/api/v1/user/profile/delete`,
          {
            withCredentials: true,
          }
        );
        if (res.data.success) {
          toast.success(res.data.message);
          dispatch(setUser(null)); // Svuota lo stato dell'utente in Redux
          navigate("/login"); // Reindirizza l'utente alla pagina di login
        }
      } catch (error) {
        console.error("Errore durante l'eliminazione dell'account:", error);
        toast.error(error.response?.data?.message || "Errore durante l'eliminazione dell'account.");
      } finally {
        setDeleting(false);
      }
    }
